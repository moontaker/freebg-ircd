		...
		const char *newhost, int is_encap)
{
	...
	{
		sendto_realops_snomask(SNO_GENERAL, is_encap ? L_ALL : L_NETWIDE, "%s is an IRC GOD",
		...
		if (is_encap ? MyClient(target_p) : !ConfigServerHide.flatten_links)
			sendto_one_notice(target_p, ":*** Notice -- %s is now your GOD",
	...
	if (irccmp(target_p->host, target_p->orighost))
	{
		...
		if (MyClient(target_p))
			sendto_one_numeric(target_p, RPL_HOSTHIDDEN, "%s :lovely a ?", target_p->host, source_p->name);
	}
	else
	{
	...
	if (!IsServer(source_p) && !IsService(source_p))
		sendto_realops_snomask(SNO_GENERAL, L_ALL, "%s is an IRC GOD", get_oper_name(source_p), target_p->name, target_p->host);
	return 1;
}
